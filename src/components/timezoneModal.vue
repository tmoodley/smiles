    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" style="font-size:15px; font-weight:bold; color:#01151C" id="exampleModalLabel">Change time zone</h5>
        ...
        <b-form>
          <div class="modal-body">
            <p style="margin-left: 3%;font-size:15px;color:#808080 ;font-weight:bold">Timezone</p>
            <b-form-group label="" label-for="ratio" label-cols-md="auto" class="mb-3">
              <b-form-select id="ratio" v-model="ShowTimezone" style="font-size:15px; font-weight:bold; color:#01151C" :options="timeZoneoptions"></b-form-select>
            </b-form-group>
          </div>
          <div class="modal-footer">
            <b-button type="button" id="cancelBtn" variant="outline-primary" data-dismiss="modal">Cancel</b-button>
            <b-button type="button" @click="UpdateTimeZone" variant="success" data-dismiss="modal">Update</b-button>
          </div>
        </b-form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    UpdateTimeZone () {
      console.log('checking the selected value')
      console.log(this.OrganizationId)
      console.log(this.ShowTimezone)
      this.updateTimezone = this.timeZoneoptions.find(item => item.value == this.ShowTimezone)
      console.log(this.updateTimezone.text) // TimeZone value to be pushed to the API
      axios
        .put('/portal/api/TimeZone/' + `${this.OrganizationId}` + `/?timeZone=${this.updateTimezone.text}`)
    },
    ...
    getUnits: function () {
      axios
        .get('/portal/api/TimeZone/' + `${this.OrganizationId}`)
        .then(response => {
          this.timezoneInfo = response.data
          console.log(this.timezoneInfo)
          ...
          if (response.statusText == 'No Content') {
            ...
            console.log(this.getTimezoneValue(this.timezoneInfo).value)
