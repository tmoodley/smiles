    ...
    <b-row no-gutters>
      <b-col cols="12" sm="12" md="12" lg="12" xl="12">
        <div class="card gedf-card">
          <div class="card-body">
            <p class="card-text">
              <b-form-group label="Please Select Subjects"
                            v-slot="{ ariaDescribedby }">
                <subject :subject="subject" v-for="subject in subjects"></subject>
              </b-form-group>
            </p>
          </div>
        </div>
...
<script>
import subject from 'components/settings/subject.vue'
...
export default {
  ...
  mounted: function () {
    ...
    this.getSubjects().then(function () {
      for (var subject of self.subjects) {
        self.options.push({ text: subject.name, value: subject.id })
      }
    })
    this.OrganizationId = JSON.parse(localStorage.getItem('organizationId'))
    if (this.company.organizationSubjects != null) {
      for (var subject of self.company.organizationSubjects) {
        self.selected.push(subject.subjectId)
      }
    }
  }
}
