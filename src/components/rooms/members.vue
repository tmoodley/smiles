...
<div>
    <b-container class="pb-6 pb-8 pt-5 pt-md-8 bg-gradient-success">
      ...
      <b-row>
        ...
        <p class="no-padding-margin sub-title">Manage your groups members from here</p>
      </b-col>
      ...
      <b-col>
        <b-table outlined responsive head-variant="light" :items="room.organizationRooms" :fields="fields">
          <template #cell(actions)="row">
            ...
            <b-button size="sm" variant="primary" @click="approve(row)" v-if="row.item.organizationId != actualOrgId && row.item.isRequest">
              Approve Access
            </b-button>
            <b-button size="sm" v-if="row.item.organizationId == actualOrgId">
              Admin
            </b-button>
          </template>
        </b-table>
